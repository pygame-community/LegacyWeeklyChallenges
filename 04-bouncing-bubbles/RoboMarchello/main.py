...
class Bubble:
    ...
    def collide(self, other: "Bubble") -> Optional["Collision"]:
        ...
        if distance - self.radius - other.radius < 0:
            positionVec = self.position - other.position
            normal = positionVec.normalize()
            normal.rotate_ip(-90)

            get_angle = atan2(positionVec.x, positionVec.y)
            DirVec = pygame.Vector2(0, -1)
            DirVec.rotate_ip_rad(-get_angle)
            
            overlappingArea = positionVec - DirVec * (self.radius + other.radius)
            center = self.position + DirVec * self.radius + overlappingArea * 0.5
...
class Collision:
    ...
    def resolve(self):
        ...
        separatingForce = self.normal.rotate_ip(90)

        massSum = self.first.mass + self.second.mass
        first2mass = self.first.mass * 2
        second2mass = self.second.mass * 2
        
        mass1Ratio = second2mass / massSum
        mass2Ratio = first2mass / massSum

        self.first.velocity += self.normal * mass1Ratio
        self.second.velocity -= self.normal * mass2Ratio
