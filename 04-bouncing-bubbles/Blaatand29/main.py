...
class Bubble:
    ...
    def collide_borders(self):
        ...
        if self.position.x < self.radius:
            strength = chrange(self.position.x, (self.radius - 200, -self.radius), (0, 1), power=1)
            self.velocity.x += strength

        # right wall
        elif self.position.x > SIZE[0] - self.radius:
            strength = chrange(self.position.x, (SIZE[0] - self.radius, SIZE[0] + 200 - self.radius), (1, 0), power=1)
            self.velocity.x -= strength

        # top wall
        if self.position.y < self.radius:
            strength = chrange(self.position.y, (self.radius - 200, -self.radius), (0, 1), power=1)
            self.velocity.y += strength

        # bottom wall
        elif self.position.y > SIZE[1] - self.radius:
            strength = chrange(self.position.y, (SIZE[1] - self.radius, SIZE[1] + 200 - self.radius), (1, 0), power=1)
            self.velocity.y -= strength

    def collide(self, other: "Bubble") -> Optional["Collision"]:
        if self.position.distance_to(other.position) <= self.radius + other.radius:
            col_point = self.position.lerp(other.position, self.radius / (other.radius + self.radius))
            normal = (self.position - other.position).normalize()
            return Collision(self, other, col_point, normal)
        return None
...
class Collision:
    ...
    def resolve(self):
        ...
        total_mass = self.first.mass + self.first.mass
        first_mass_con = self.first.mass / total_mass
        second_mass_con = self.second.mass / total_mass

        reflecta = self.first.velocity.reflect(self.normal)
        reflectb = self.second.velocity.reflect(self.normal)

        self.first.velocity = (self.first.velocity * first_mass_con + reflecta * second_mass_con) - -self.normal * reflectb.length() * second_mass_con
        self.second.velocity = (self.second.velocity * second_mass_con + reflectb * first_mass_con) - self.normal * reflecta.length() * first_mass_con
