...
def generate_light(player):
    ...
    for opacity_level in range(0, MAX_DARKNESS_LEVEL, 5):
        pygame.draw.circle(light, (0, 0, 0, opacity_level), light.get_rect().center, player.VISION_RADIUS,
                           int(player.VISION_RADIUS * (1 - opacity_level / MAX_DARKNESS_LEVEL)))
    pygame.draw.circle(light, (0, 0, 0, MAX_DARKNESS_LEVEL), light.get_rect().center, light.get_width(),
...
def update_unvisited_chunks(player, unvisited_chunks):
    for chunk in set(unvisited_chunks):
        chunk_rect = pygame.rect.Rect(chunk[0] * CHUNK_SIZE[0], chunk[1] * CHUNK_SIZE[1], CHUNK_SIZE[0],
                                      CHUNK_SIZE[1])
        player_distance_with_chunk = math.sqrt((chunk_rect.center[0] - player.rect.center[0]) ** 2 + (
...
def display(screen, objects, player, light, unvisited_chunks):
    ...
    for element in sorted(objects, key=attrgetter("rect.bottom")):
        if isinstance(element, Ghost) and element.pos.distance_to(player.pos) > player.VISION_RADIUS:
    ...
    for chunk_position in unvisited_chunks:
        ...
        screen.fill(pygame.Color("black"),
                    rect=pygame.rect.Rect(chunk_position[0] * CHUNK_SIZE[0], chunk_position[1] * CHUNK_SIZE[1],
                                          CHUNK_SIZE[0], CHUNK_SIZE[1]))

    screen.blit(light, (player.rect.center[0] - light.get_width() // 2,
...
def mainloop():
    ...
    player_chunk = (player.rect.center[0] // CHUNK_SIZE[0], player.rect.center[1] // CHUNK_SIZE[1])
