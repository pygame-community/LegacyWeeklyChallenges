    ...
    def _render_noise(self, surf, severity):
        ...
        for x in range(r, self._light_sphere_w, r):
            for y in range(r, self._light_sphere_w, r):
                ...
                noise_c = (min(c1+noise_add, 255), min(c2+noise_add, 255), min(c3+noise_add, 255))
...
def tile_paint( graph_grid, flag_grid, pos, paint_flag=TILE_POS_ALL ):
    ...
    col, row = pos
    pos_idx = 0
    pbrush = [TILE_POS_SE, TILE_POS_S, TILE_POS_SW,
              TILE_POS_E, paint_flag, TILE_POS_W,
              TILE_POS_NE, TILE_POS_N, TILE_POS_NW]
...
def mainloop():     
    ...
    while True:
        ...
        if (tile_col != old_tile_col or tile_row != old_tile_row) and not fog_t_visited[tile_col, tile_row]:            
        ...
        old_tile_col, old_tile_row = fog_t_graph.cell_at_pos( (player.rect.centerx, player.rect.centery) )  
        ...
        for object in tuple(sorted(all_objects, key=attrgetter("rect.bottom"))):            
            if isinstance(object, Ghost):
                if not in_circle(object.rect.x, object.rect.y, radius, player.rect.x, player.rect.y):
                    continue
            object.draw(screen)

        fog_surface.fill(light_spot._fog_color)
      
        pygame.draw.rect(fog_surface, light_spot.light_color, (player.rect.centerx-LIGHT_SPOT_SIZE//2, player.rect.centery-LIGHT_SPOT_SIZE//2, LIGHT_SPOT_SIZE, LIGHT_SPOT_SIZE))                            

        p_rect = player.rect
   
        all_objects_filtered = sorted( (obj for obj in all_objects if not obj is player and 
                                    not obj.rect.colliderect(p_rect) and rect_dist(player.rect, obj.rect)<cull_distance), key=lambda obj: player.pos.distance_squared_to(obj.pos), reverse=True)
        ...
        for obj in all_objects_filtered:
            ...
            shadow_len = int(obj.rect.height*.15+Vector2(obj_rect.center).distance_to(player_c_pos)//3)
            ...
            subrects1 = sorted([sr for sr, _ in subrects], key=lambda r: Vector2(r.center).distance_squared_to(player_c_pos))
            ...
            angle = (int(measure_angle_vec(Vector2( obj_rect.center ), player_c_pos))//angle_step)*angle_step
            ...
            for a_i in range(0,360, angle_step):
                if not raycast_ended_r:
                    ...
                    if colis:
                        ...
                        line_bk = line[1], line[0] # backwards light ray to create shadow outer contour            
                        for s_rect in reversed(subrects1): 
                            ...
                            if colis2:
                                ...
                                # shadow length                                                
                                p2_off = Vector2(shadow_len, 0).rotate(ang1) # easier and faster rotation with vectors                                        
                                p2 = tuple(p2 + p2_off)
                                ...
                                if cached is not None:
                                    ...
                                    cached_p1_dist = Vector2(cached[0]).distance_squared_to(player_c_pos) 
                                    cached_p2_dist = Vector2(cached[1]).distance_squared_to(player_c_pos)
                                    is_inside = pt1_dist>cached_p1_dist and pt2_dist<cached_p2_dist
                                    is_outside = pt2_dist<cached_p1_dist or pt1_dist>cached_p2_dist
        ...
        #fake blurring
        if BLUR_SHADOWS:
            the_pos = player.rect.centerx-LIGHT_SPOT_SIZE//2, player.rect.centery-LIGHT_SPOT_SIZE//2            
            lighto.blit(fog_surface, (0,0), (*the_pos, LIGHT_SPOT_SIZE, LIGHT_SPOT_SIZE))
            if BLUR_SHADOWS>1:
                l_blurred = pygame.transform.smoothscale(lighto, (LIGHT_SPOT_SIZE//4, LIGHT_SPOT_SIZE//4))
            l_blurred = pygame.transform.smoothscale(l_blurred,(LIGHT_SPOT_SIZE//2, LIGHT_SPOT_SIZE//2))
