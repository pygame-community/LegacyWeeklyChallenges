...
class Button:
    def __init__(self, position, size, content: str, bgColor,nPath,cellSize,funcs,icon:pygame.Surface=None):  # Just an example!
    ...
    def nine_path(self,screen):
        ...
        for tilex in range(self.tiles[0]-2):
            ...
            for tiley in range(self.tiles[1]-2):
                ...
                screen.blit(transx,[self.rect.left+self.tiles[0]*15-15,self.rect.top+tileYPos])
            ...
            screen.blit(transy,[self.rect.left+tileXPos+15,self.rect.top+self.tiles[1]*15-15])

        screen.blit(pygame.transform.scale(self.nPath[4],(transWidth,transHeight)),[self.rect.left+self.tiles[0]*15-15,self.rect.top+self.tiles[1]*15-15])
        
        #draw transformed borders
        transBorderx = self.rect.left+self.tiles[0]*15-15
        transBordery = self.rect.left+self.tiles[0]*15-15
        screen.blit(pygame.transform.scale(self.nPath[3],(self.cellSize[0], transHeight)),[self.rect.left, self.rect.top+self.tiles[1]*15-15])
        screen.blit(pygame.transform.scale(self.nPath[5],(self.cellSize[0], transHeight)),[self.rect.right-15 ,self.rect.top+self.tiles[1]*15-15])
        screen.blit(pygame.transform.scale(self.nPath[1],(transWidth, self.cellSize[1])),[self.rect.left+self.tiles[0]*15-15, self.rect.top])
        screen.blit(pygame.transform.scale(self.nPath[7],(transWidth, self.cellSize[1])),[self.rect.left+self.tiles[0]*15-15, self.rect.bottom-15])
...
class ripple():
    def __init__(self,pos,color,ripps):
        ...
        for col in self.color:
            self.color[self.color.index(col)] = color[self.color.index(col)]+int(col*0.2)*ripps
...
class circleButton(Button):
    def __init__(self, position, size, content: str, bgColor,nPath,cellSize,funcs,icon:pygame.Surface=None):
    ...
    def on_hover(self):
        ...
        pygame.draw.circle(self.hoverSurf,self.bgColor,[self.mousePos[0]-self.position[0],self.mousePos[1]-self.position[1]],self.hoverRadius)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                if self.hovered:
                    ...
                    self.ripples.append(ripple([self.mousePos[0]-16,self.mousePos[1]-16],self.bgColor,self.rippleColor))
...
class perlinButton(Button):
    def __init__(self, position, size, content: str, bgColor,nPath,cellSize,funcs,icon:pygame.Surface=None):
        ...
        self.perlinNoise = pygame.image.load('03-buttons/RoboMarchello/noise_transition.png').convert()
        self.crntCol = 100
    ...
    def on_hover(self):
        ...
        mask = pygame.mask.from_threshold(self.perlinNoise, (0,0,0), threshold=(self.crntCol,self.crntCol,self.crntCol,255))
...
def mainloop():
    ...
    buttons = [
        circleButton((20, 20), (200, 100), "Click me!",(240, 138, 93),SplitSheet(pygame.image.load('03-buttons/RoboMarchello/nine_path.png').convert_alpha(),(15,15)),(15,15),[func1,func2,func3],icon=pygame.image.load('03-buttons/RoboMarchello/icon.png').convert()),
        perlinButton((20, 150), (200, 100), "Click me!",(184, 59, 94),SplitSheet(pygame.image.load('03-buttons/RoboMarchello/nine_path.png').convert_alpha(),(15,15)),(15,15),[func1,func2,func3]),
        Button((250, 20), (100, 230), "Play!",(247, 228, 77),SplitSheet(pygame.image.load('03-buttons/RoboMarchello/grass_ninepath.png').convert_alpha(),(15,15)),(15,15),[func1,func2,func3]),
