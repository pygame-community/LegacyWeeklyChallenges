...
class FpsCounter(Object):
    ...
    def draw(self, screen):
        ...
        t = text(f"Particles: {len(PARTICLES)}", color)
        screen.blit(t, (self.center.x, self.center.y + 20))


# class Particles(Object):
#     def __init__(self, pos, vel, colour, size):
#         self.size = random.randint(size[0], size[1])
#         self.surf = pygame.Surface((self.size, self.size))
#         self.surf = pygame.draw.circle(self.surf, colour, self.center, self.size // 2)
#         super.__init__(pos, vel, self.surf)
