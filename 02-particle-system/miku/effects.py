...
class CrashParticle(Particle):
	...
	def __init__(self, position=(0, 0), pieces=20, lifetime=300, speed=15, surface=None):
		...
		for i in range(pieces):
			...
			self.mini_surfaces.append([[self.center[0]-x, self.center[1]-y], new_surf, angle, 0])
			#											0						1		 2		 3
...
class TrailParticle(Particle):
	@lru_cache(maxsize=2000, typed=True)
	def __init__(self, position=(0, 0), offset=(0, 0), radian=1, speed=1, lifetime=300, 
				 transparent=False, alpha=255, random_rotation=False, center_position=False, surface=None):
		super().__init__(position, lifetime, surface)
		self.speed = 1
		self.lifetime = lifetime
		self.surface = surface
		...
		self.position = [pos[0]+random.gauss(-offset[0], offset[0]), pos[1]+random.gauss(-offset[1], offset[1])]
...
class SimpleParticle(Particle):
	@lru_cache(maxsize=2000,typed=True)
	def __init__(self, position=(0, 0), offset=(0, 0), lifetime=300, 
				color=(255, 255, 255), size=50, bold=0, dark_over_time=0, change_size_over_time=0):
		...
		pos = [*position]
		self.position = (pos[0]+random.randint(-offset[0], offset[0]), pos[1]+random.randint(-offset[1], offset[1]))
