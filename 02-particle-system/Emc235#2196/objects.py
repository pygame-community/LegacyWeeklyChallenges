...
from .ParticleSys import Particles
...
class State:
    ...
    def get_asteroids(self) -> List["Asteroid"]:
        return [obj for obj in self.objects if isinstance(obj, Asteroid)]
...
class Asteroid(Object):
    ...
    def explode(self, bullet):
        ...
        # the breaking of the asteroid
        surfaces = split_surface(self.rotated_sprite)
        i = 0
    ...
    def random_color(self):
        r, g, b = hsv_to_rgb(random.random(), 0.8, 0.8)
...
class FpsCounter(Object):
    ...
    def draw(self, screen):
        ...
        color = "#89C4F4"
        particle_count = text(f"particles count: {particles.length}", color)
        screen.blit(particle_count, self.center)
        fps_count = text(f"FPS: {int(self.current_fps)}", color)
