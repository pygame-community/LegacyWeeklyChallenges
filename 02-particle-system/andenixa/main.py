...
class Cracklet:
    ...
    def propagate(self):        
        ...
        step = pygame.Vector2(self.size+random.randint(-self.size//2,self.size//2)).rotate(self.angle + random.randint(-self._angle_random-rnd_add, self._angle_random+rnd_add))
        #step = pygame.Vector2(10).rotate(self.angle)
        step = last_p + step
        self.points += [step]

class AsteroidEx(Asteroid):
    _blinky_color = [(127, 127, 113), (119, 118, 101), (93, 93, 77), (55, 55, 44), (15, 15, 12), (0, 0, 0), (41, 41, 29), 
                     (143, 142, 96), (242, 202, 97), (226, 179, 85), (79, 60, 28), (2, 2, 0), (149, 100, 47), (248, 157, 73), 
                     (65, 38, 17), (44, 24, 11), (226, 77, 30), (64, 21, 8), (62, 20, 8), (178, 56, 21), (0, 0, 0), 
                     (156, 45, 17), (18, 5, 1), (113, 29, 10)]    
    ...
    def explode(self, bullet):
        ...
        if not (cracks := generate_cracks(collision_point, asteroid_rect, p_angle=b_angle)):
            return
        ...
        self.state.particle_man.spawnMany(40, self_xy+collision_point, bullet.vel.dot(self.vel)*3, 350, Particles.TYPE_SIMPLE, spawn_cb=impact_cb            )
        self.state.particle_man.spawnMany(20, self_xy+collision_point, bullet.vel.dot(self.vel)*3, 450, Particles.TYPE_SIMPLE, spawn_cb=impact_cb,
            color=self._blinky_color)
        self.state.particle_man.spawnMany(sum(asteroid_rect.size)//2, self.center, self.vel, 150, Particles.TYPE_SNOWFLAKE, 
            spawn_cb=lambda p: asteroid_explosion(p, self), color=((255,254,226), (255,254,164), (255,213,103), (252,128,58), (229,78,31), (125,30,10)))              
        ...
        for crack in cracks:            
            for i in range(len(crack.points)):
                if i==0:
                    continue                
                pygame.draw.line(asteroid1, (0,0,0), crack.points[i-1], crack.points[i], width=2)
    
        asteroid.set_colorkey(colorkey)        
        m = pygame.mask.from_surface(asteroid1)

        for (shard_img, shard_mask, r) in break_surface(asteroid, m):
            ratio = (mm_count:=shard_mask.count())/m.count()
            sh_speed = pygame.Vector2(((r.centerx - a_rect.centerx)*(1-ratio), (r.centery-a_rect.centery)*(1-ratio))) 
            sh_speed+=bullet.vel*4
            sh_pos = pygame.Vector2( (asteroid_x+r.x, asteroid_y+r.y) )
            # if bigger than a size of smallest astroid                         
            if mm_count > 840:
                self.state.add(
                        AsteroidEx(sh_pos+shard_img.get_rect().center, sh_speed/15, shard_img, self.color)
                    )
            else:
                self.state.particle_man.spawn(sh_pos, sh_speed, 800+rnd()*150, p_type=Particles.TYPE_SPRITE, 
                                              surf=shard_img, color=(255,255,255), torque=random.randint(-15, 16))
...
class FpsCounterEx(FpsCounter):
    ...
    def draw(self, screen):        
        ...
        screen.blit(self._particles_t, (t_pos:=self._part_t_pos))
        screen.blit(particle_count_t, t_pos + (self._particles_t_w+8,0))
        super().draw(screen)

def generate_cracks(st_point, bounding_rect, max_cracks=7, crack_len=7, p_angle=None, 
    angle_proclivity=1.0, new_crack_chance=15, chance_to_split=50, cracks_spread=90):
    """ 
    ...
    while True:
        ...
        for crack in cracks:
            ...
            if new_crack_chance>(rnd()*100):
                ...
                new_cracks += [Cracklet(pt, crack.angle+angle_add, bounding_rect, size=(crack_len-2))]
                if chance_to_split>(rnd()*100): # 50% chance to kill the old crack, i.e. spl.splitting
                    new_cracks += [Cracklet(pt, crack.angle-angle_add+random.randint(-5,5), bounding_rect, size=(crack_len-2))]                        
                    crack.stop()
        if done: break
...
def mainloop():
    ...
    fire_colors1 = (255,254,226), (255,254,164), (255,213,103), (252,128,58), (229,78,31), (125,30,10)
    ...
    engine_exhaust = particle_man.addSpawner(15,(0,0), (0,0),150, Particles.TYPE_SIMPLE, spawn_cb=burner_spawn_cb, color=fire_colors2)   
    engine_exhaust_blinky =  particle_man.addSpawner(150,(0,0), (0,0),350, Particles.TYPE_SIMPLE, spawn_cb=burner_spawn_cb, color=AsteroidEx._blinky_color)   
