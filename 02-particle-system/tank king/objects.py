...
from .utils import *
from .utils import map_to_range, distance
...
class Player(Object):
    ...
    def logic(self):
        # print(len([i for i in self.state.objects if isinstance(i, ExhaustParticles)]))
        self.fire_cooldown -= 1
...
class ExhaustParticles(Object):
    ...
    def logic(self, **kwargs):
        # self.original_size -= random.randint(1, 5) / 40
        # self.size = int(self.original_size)
        # self.sprite.set_alpha(map_to_range(self.size, 0, self.real_size, 0, 255))
        # if self.size < 1:
        #     self.size = 1
        #     self.alive = False
        # self.sprite = pygame.transform.scale(self.sprite, (self.particle_sprite.get_width() * self.size // 3, self.particle_sprite.get_height() * self.size // 3))
